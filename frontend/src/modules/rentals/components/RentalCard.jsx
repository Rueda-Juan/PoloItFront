...
// Recibe la nueva prop 'onEditClick'
function RentalCard({ rental, onEditClick }) {
  ...
  const handleEdit = (e) => {
    e.preventDefault();
    e.stopPropagation();
    // En lugar de navegar, llama a la función del padre (MyRentalsPage)
    // y le pasa el objeto 'rental' completo para abrir el modal.
    if (onEditClick) {
      onEditClick(rental);
    }
  };
  ...
  return (
    <div className="bg-white shadow-lg rounded-lg overflow-hidden flex flex-col justify-between transform transition-transform duration-300 hover:scale-105">
